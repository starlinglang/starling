...
function requireTokens () {
  ...
  tokens.createTokenArray = createTokenArray

  return tokens
}

let hasRequiredCheckmm
...
function requireCheckmm () {
  ...
  (function (exports$1) {
    ...
    const __createBinding =
      (checkmm$1 && checkmm$1.__createBinding) ||
      (Object.create
        ? function (o, m, k, k2) {
    ...
    const __setModuleDefault =
      (checkmm$1 && checkmm$1.__setModuleDefault) ||
      (Object.create
        ? function (o, v) {
    ...
    const __importStar =
      (checkmm$1 && checkmm$1.__importStar) ||
      (function () {
    ...
    const __importDefault =
      (checkmm$1 && checkmm$1.__importDefault) ||
      function (mod) {
    ...
    Object.defineProperty(exports$1, 'TokenArray', {
      enumerable: true,
      get: function () {
        return tokens_1.TokenArray
      }
    })
    ...
    let labelused = (label) => {
      return (
        hypotheses.get(label) !== undefined ||
        assertions.get(label) !== undefined
      )
    }
    ...
    let readtokens = (filename_1, ...args_1) =>
      __awaiter(
        void 0,
        [filename_1, ...args_1],
        void 0,
        function * (filename, lastFileInclusionStart = 0) {
          const alreadyencountered =
            mmfilenamesalreadyencountered.has(filename)
          if (alreadyencountered) return
          mmfilenamesalreadyencountered.add(filename)
          try {
            data =
              data.slice(0, lastFileInclusionStart) +
              (yield readFile(filename)) +
              data.slice(dataPosition)
          ...
          for (;;) {
            ...
            if (fileInclusion) {
              if (path_1.default) {
                fileInclusion.filename = path_1.default.normalize(
                  path_1.default.join(
    ...
    let getproofnumbers = (label, proof) => {
      ...
      for (const item of proof) {
        if (item <= 'T') {
          ...
          if (
            num > Number.MAX_SAFE_INTEGER / 20 ||
            20 * num > Number.MAX_SAFE_INTEGER - addval
          ) {
            throw new Error(
              'Overflow computing numbers in compressed proof of ' + label
            )
          }
          ...
          if (
            num > Number.MAX_SAFE_INTEGER / 5 ||
            5 * num > Number.MAX_SAFE_INTEGER - addval
          ) {
            throw new Error(
              'Overflow computing numbers in compressed proof of ' + label
            )
      ...
      if (num !== 0) {
        throw new Error(
          'Compressed proof of theorem ' + label + ' ends in unfinished number'
        )
      }
      return proofnumbers
    }
    ...
    let verifyassertionref = (thlabel, reflabel, stack) => {
      ...
      if (stack.size() < assertion.hypotheses.length) {
        throw new Error(
          'In proof of theorem ' + thlabel + ' not enough items found on stack'
        )
      }
      ...
      for (let i = 0; i < assertion.hypotheses.length; ++i) {
        ...
        if (hypothesis.second) {
          ...
          if (hypothesis.first[0] !== stack.at(base + i)[0]) {
            throw new Error(
              'In proof of theorem ' + thlabel + ' unification failed'
            )
          }
    ...
    let verifyregularproof = (label, theorem, proof) => {
      ...
      if (!std.arraysequal(stack.at(0), theorem.expression)) {
        throw new Error(
          'Proof of theorem ' + label + ' proves wrong statement'
        )
      }
    }
      ...
      for (const item of proofnumbers) {
        ...
        if (item <= mandhypt) {
          ...
          if (item > labelt + savedsteps.length) {
            throw new Error(
              'Number in compressed proof of ' + label + ' is too high'
            )
          }
      ...
      if (tokens.front() === '(') {
        ...
        while (!tokens.empty() && (token = tokens.front()) !== ')') {
          ...
          if (token === label) {
            ...
            throw new Error(
              'Compressed proof of theorem ' +
                label +
                ' has mandatory hypothesis ' +
                token +
                ' in label list'
            )
        ...
        while (!tokens.empty() && (token = tokens.front()) !== '$.') {
          ...
          if (!containsonlyupperorq(token)) {
            throw new Error(
              'Bogus character found in compressed proof of ' + label
            )
          }
        }
        if (tokens.empty()) {
        ...
        if (proof.includes('?')) {
          console.error(
            'Warning: Proof of theorem ' + label + ' is incomplete'
          )
      ...
      while (!tokens.empty() && (token = tokens.front()) !== '$.') {
        ...
        if (!isactivevariable(token)) {
          throw new Error(
            'Token ' +
              token +
              ' is not an active variable, ' +
              'but was found in a $d statement'
          )
        }
        if (dvars.has(token)) {
    ...
    let parsec = () => {
      ...
      while (!tokens.empty() && (token = tokens.front()) !== '$.') {
        ...
        if (variables.has(token)) {
          throw new Error(
            'Attempt to redeclare variable ' + token + ' as a constant'
          )
        }
        if (labelused(token)) {
    ...
    let parsev = () => {
      ...
      while (!tokens.empty() && (token = tokens.front()) !== '$.') {
        ...
        if (constants.has(token)) {
          throw new Error(
            'Attempt to redeclare constant ' + token + ' as a variable'
          )
        }
        if (labelused(token)) {
...
const checkmmExports = requireCheckmm()
const checkmm = /* @__PURE__ */ getDefaultExportFromCjs(checkmmExports)
...
const stateExports = requireState()

const tokensExports = requireTokens()
